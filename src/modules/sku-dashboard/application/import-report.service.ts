...
function resolve(row: Record<string, string>, field: string): string {
  const normalizedRow = Object.entries(row).map(
...
export class ImportReportService implements IImportReportUseCase {
  ...
  async execute(command: ImportReportCommand): Promise<ImportReportResult> {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        const productInput: UpsertProductInput = {
          ...
          category: resolve(row.values, 'category') || null,
        ...
        const stockLevel =
          parseNum(resolve(row.values, 'stockLevel')) ?? available;
        ...
        // ---- Sales Metrics --------------------------------------------------
        const metricChannel = mapChannel(
          channelRaw || (asin ? 'AMAZON' : 'OTHER'),
        );
        ...
        this.logger.warn(
          `Row ${row.rowNumber} (SKU: ${sku}) failed: ${reason}`,
        );
        rowErrors.push({
          batchId,
          rowNumber: row.rowNumber,
    ...
    this.logger.log(
      `Import ${batchId}: ${importedRows} imported, ${failedRows} failed`,
    );
