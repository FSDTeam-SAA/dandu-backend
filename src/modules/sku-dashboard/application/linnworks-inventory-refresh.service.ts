...
export class LinnworksInventoryRefreshService {
  ...
  async refreshInventory(): Promise<InventoryRefreshResult> {
    ...
    try {
      ...
      const remainingSkus = [
        ...new Set(stockItems.map((item) => item.ItemNumber).filter(Boolean)),
      ];
      ...
      for (const item of stockItems) {
        ...
        try {
          ...
          failedRows++;
          this.logger.warn(
            `Failed to refresh product ${item.ItemNumber}: ${(err as Error).message}`,
          );
        }
      }
      ...
      for (const level of stockLevels) {
        ...
        try {
          ...
          failedRows++;
          this.logger.warn(
            `Failed to refresh stock for ${sku}: ${(err as Error).message}`,
          );
        }
      }
      ...
      for (const listing of channelListings) {
        ...
        try {
          ...
          failedRows++;
          this.logger.warn(
            `Failed to refresh listing for ${sku}: ${(err as Error).message}`,
          );
        }
      }

      const cleanup =
        await this.skuRepository.deleteProductsNotInSkus(remainingSkus);
